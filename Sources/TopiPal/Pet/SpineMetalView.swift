...
struct SpineMetalView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, MTKViewDelegate {
        ...
        func attach(to view: MTKView) {
            ...
            descriptor.vertexFunction = library?.makeFunction(name: "spine_vertex")
            descriptor.fragmentFunction = library?.makeFunction(name: "spine_fragment")
        ...
        func load(character: PetCharacter, in view: MTKView) {
            ...
            guard let atlasURL,
                  let skeletonURL,
                  let nextDrawable = topi_spine_create(atlasURL.path, skeletonURL.path, 1.0) else {
                characterID = nil
                textures = []
                return
            }
            ...
            if let idle = findAnimation(in: nextDrawable, containing: "idle") {
                currentAction = idle
                topi_spine_set_animation(nextDrawable, idle, 1)
            } else if let first = firstAnimationName(in: nextDrawable) {
                currentAction = first
                topi_spine_set_animation(nextDrawable, first, 1)
            }

            // Capture reference bounds from the idle pose to keep centering stable across animations
            topi_spine_update(nextDrawable, 0)
            let vCount = Int(topi_spine_vertex_count(nextDrawable))
            if vCount > 0, let vPtr = topi_spine_vertices(nextDrawable) {
        ...
        func setAction(_ action: String?) {
            ...
            topi_spine_set_animation(drawable, resolved, 1)
        }
        ...
        func draw(in view: MTKView) {
            ...
            lastFrameDate = now
            topi_spine_update(drawable, Float(delta))

            let vertexCount = Int(topi_spine_vertex_count(drawable))
            let indexCount = Int(topi_spine_index_count(drawable))
            let commandCount = Int(topi_spine_command_count(drawable))
            guard vertexCount > 0, indexCount > 0, commandCount > 0,
                  let vertexPointer = topi_spine_vertices(drawable),
                  let indexPointer = topi_spine_indices(drawable),
                  let commandPointer = topi_spine_commands(drawable),
                  let device = view.device,
                  let vertexBuffer = device.makeBuffer(
                    bytes: vertexPointer,
                    length: MemoryLayout<TopiSpineVertex>.stride * vertexCount
                  ) else {
                return
            }
            ...
            encoder.setVertexBytes(&mutableUniforms, length: MemoryLayout<SpineUniforms>.stride, index: 1)
        ...
        private func loadTextures(for drawable: OpaquePointer, atlasDirectory: URL, device: MTLDevice) -> [MTLTexture] {
            ...
            let count = Int(topi_spine_texture_count(drawable))

            return (0..<count).compactMap { index in
                guard let namePointer = topi_spine_texture_name(drawable, Int32(index)) else { return nil }
        ...
        private func firstAnimationName(in drawable: OpaquePointer) -> String? {
            ...
            guard count > 0, let name = topi_spine_animation_name(drawable, 0) else { return nil }
        ...
        private func findAnimation(in drawable: OpaquePointer, containing keyword: String) -> String? {
            let count = Int(topi_spine_animation_count(drawable))
            for i in 0..<count {
                if let name = topi_spine_animation_name(drawable, Int32(i)) {
        ...
        private func vertexBounds(vertices: UnsafePointer<TopiSpineVertex>, count: Int) -> CGRect {
        ...
        private func makeUniforms(bounds: CGRect, drawableSize: CGSize) -> SpineUniforms {
        ...
        vertex SpineVertexOut spine_vertex(
            const device SpineVertexIn *vertices [[buffer(0)]],
            constant SpineUniforms &uniforms [[buffer(1)]],
        ...
        fragment float4 spine_fragment(
